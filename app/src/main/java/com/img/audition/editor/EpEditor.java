package com.img.audition.editor;
...
public class EpEditor {
    ...
    private  final int DEFAULT_WIDTH = 480;//默认输出宽度
    private  final int DEFAULT_HEIGHT = 360;//默认输出高度
     ...
     */
    public  void mergeByLc(Context context, List<EpVideo> epVideos, OutputOption outputOption,  OnEditorListener onEditorListener) {
     ...
     * @param onEditorListener 回调监听
     */
    public  void demuxer(String videoin, String out, VideoHandle.EpEditor.Format format, OnEditorListener onEditorListener) {
        CmdList cmd = new CmdList();
        cmd.append("ffmpeg").append("-y").append("-i").append(videoin);
        switch (format) {
            case MP3:
     ...
     * @param onEditorListener 回调监听
     */
    public  void reverse(String videoin, String out, boolean vr, boolean ar, OnEditorListener onEditorListener) {
        if (!vr && !ar) {
     ...
     */
    public  void changePTS(String videoin, String out, float times, VideoHandle.EpEditor.PTS pts, OnEditorListener onEditorListener) {
     ...
     * @param onEditorListener	回调接口
     */
    public  void video2pic(String videoin, String out, int w, int h, float rate, OnEditorListener onEditorListener) {
        if (w <= 0 || h <= 0) {
     ...
     * @param onEditorListener	回调接口
     */
    public  void pic2video(String videoin, String out, int w, int h, float rate, OnEditorListener onEditorListener) {
        if (w < 0 || h < 0) {
            Log.e("ffmpeg", "width and height must greater than 0");
            onEditorListener.onFailure();
            return;
...
//				.append("-b").append("10M");
    ...
    public  class OutputOption {
         final int ONE_TO_ONE = 1;// 1:1
         final int FOUR_TO_THREE = 2;// 4:3
         final int SIXTEEN_TO_NINE = 3;// 16:9
         final int NINE_TO_SIXTEEN = 4;// 9:16
         final int THREE_TO_FOUR = 5;// 3:4
     ...
     * @param onEditorListener 回调接口
     */
    public void execCmd(String cmd, long duration, OnEditorListener onEditorListener) {
        cmd = "ffmpeg " + cmd;
     ...
     * @param onEditorListener 回调接口
     */
    private void execCmd(CmdList cmd, long duration, OnEditorListener onEditorListener) {
